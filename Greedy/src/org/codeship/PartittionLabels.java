...
public class PartittionLabels {
    ...
    public List<Integer> partitionLabels(String s) {
        ...
        while(i < n) {
            ...
            // [0, 8] [9, 15] [16, 23]
            // Step 2: Expand partition until we include all last occurrences of chars seen so far
            while(j < lastIndex) {
                char ch = s.charAt(j);
                int key = ch - 'a';
                //updating last index for curr char in the partition so that we can increas the size of curr partition
